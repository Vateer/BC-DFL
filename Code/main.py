...
import torch, copy, os, time, json, math, random
import numpy as np
import logging
import IPFS
import blockchain
...
class BCManager():
    def __init__(self) -> None:
        ...
        # myEth2 = MyEth(8546)
        names, abis, addresses = blockchain.MyTools.load_contract_info(r".\blockchain_multi_point\contracts", True)
    ...
    def train(self):
        ...
        for round_idx in range(self.args.comm_round):
            ...
            for idx, client in enumerate(self.client_list):
                ...
                time.sleep(1)
                print(os.path.exists('./cache/weight.pt'))
                ...
                bc_handler.send_parameter('aggregate',hash, client.get_sample_number(), str(client_idx))
            ...
            for idx in range(int(querry.__len__()/3)):
                # print(int(querry.__len__()/2))
                hash = querry[idx*3]
                if hash == "?":
                    print("catch ?")
                    continue
                sample_number = int(querry[idx*3+1])
                uploader_querry.append(int(querry[idx*3+2]))
                try:
            ...
            for idx in range(w_locals.__len__()):
                ...
                for idx2 in range(w_locals.__len__()):
                    client.update_local_dataset(0, self.train_data_local_dict[uploader_querry[idx2]],self.test_data_local_dict[uploader_querry[idx2]],self.train_data_local_num_dict[uploader_querry[idx2]])
                    # test data
                ...
                bc_handler=self.bc_manager.get_blockchain_handler(0)
                self.bc_manager.maintain_handler(0)
                bc_handler.log_acc('reward',round_idx, str(uploader_querry[idx]), int(acc*100))
                print("client {} acc = {}".format(uploader_querry[idx], acc))
            ...
            for acc, idx in acc_locals:
                ...
                if loss > delta:
                    if loss > delta + 2:
                        ...
                        self.rewards[uploader_querry[idx]] -= math.exp(0.15*loss)
                        flag = 1
                    else:
                        self.rewards[uploader_querry[idx]] += loss
                else:
                    loss = -loss
                    if loss > delta + 5:
                        self.rewards[uploader_querry[idx]] += math.exp(0.05*loss)
                    else:
                        self.rewards[uploader_querry[idx]] += abs(loss)
                if flag == 0:
...
if __name__ == '__main__':
    ...
    device = myfedml.device.get_device(args)

    dataset, output_dim = myfedml.data.load(args)

    model = myfedml.model.create(args, output_dim)
